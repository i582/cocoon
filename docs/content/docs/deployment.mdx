...
- Use [`scripts/setup-gpu-vfio`](https://github.com/TelegramMessenger/cocoon/blob/main/scripts/setup-gpu-vfio) to configure GPU for passthrough

## Quick Start

If you want just to run a worker, please see the [repository README](https://github.com/TelegramMessenger/cocoon/blob/main/README.md). The following instructions are mostly for developers.
...
What you need for a start is a TON config with reliable list of liteservers. You
should put this file in `spec/mainnet-full-ton-config.json`. Pass it via
`--ton-config <path>` to `cocoon-launch`.
...
- For RA-TLS: [RA-TLS](ra-tls.mdx)
- For GPU setup: [GPU](gpu.mdx)
