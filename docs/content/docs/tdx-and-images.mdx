...
description: Intel TDX fundamentals, guest image structure, measurements, and the COCOON boot and verification flow.
---
...
<Files>
  <File name="OVMF.fd" description="TDX-enabled UEFI firmware (→ MRTD)" />
  <File name="vmlinuz" description="Linux kernel (→ RTMR[1])" />
  <File name="initrd" description="Initial ramdisk (→ RTMR[2])" />
  <File
    name="cmdline"
    description="Kernel boot parameters, includes rootfs hash (→ RTMR[2])"
  />
  <File
    name="rootfs.img"
    description="Root filesystem (read-only, dm-verity protected)"
  />
</Files>
...
We use **mkosi** with **Debian** for reproducible builds. The build process is defined in [`reprodebian/mkosi.conf`](https://github.com/TelegramMessenger/cocoon/blob/main/reprodebian/mkosi.conf) and related configuration files.
...
QEMU starts the TDX VM with the following key components (see [`scripts/cocoon-launch`](https://github.com/TelegramMessenger/cocoon/blob/main/scripts/cocoon-launch) for full setup):
...
The initialization script ([`reprodebian/cocoon-init/cocoon-init`](https://github.com/TelegramMessenger/cocoon/blob/main/reprodebian/cocoon-init/cocoon-init)) runs as the first service and performs the following steps:
...
<Files>
  <Folder name="/spec" defaultOpen>
    <File name="worker-config.json" description="Worker settings" />
    <File name="init-engines.sh" description="Startup script" />
    <File name=".gitignore" description="Files to exclude from hash" />
    <Folder name="runtime" description="Not measured (see below)" />
  </Folder>
</Files>
...
<Files>
  <Folder name="/spec/runtime" defaultOpen>
    <File
      name="runtime.vars"
      description="Runtime variables for config templates"
    />
    <File
      name="global.config.json"
      description="TON config with liteserver endpoints"
    />
  </Folder>
</Files>
...
See [RA-TLS documentation](ra-tls.mdx) for how quotes are embedded in TLS certificates.
...
1. Persistent key is derived during initialization (see [Seal Keys](seal-keys.mdx))
...
- For RA-TLS (using quotes in TLS): [RA-TLS](ra-tls.mdx)
