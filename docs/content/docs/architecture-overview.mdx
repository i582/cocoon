...
<Callout type="info">
  For a complete technical overview, this page should be read alongside the
  in-depth technical documentation in this docs set.
</Callout>
...
<Callout type="info">
  Both proxy and worker run inside TEE, ensuring all data (prompts, responses)
  remains private and cannot be accessed by server owners.
</Callout>
...
<Callout type="idea">
  Future: Anyone will be able to run their own proxy, just like workers,
...
<Callout type="info">
  Note: The client library is designed to run on servers and backend
  infrastructure. For example, the Telegram backend would run multiple client
  instances to handle user requests.
</Callout>
...
<Callout type="info">
  Workers connect to proxies. Proxies track worker reputation and load-balance
  requests.
</Callout>
