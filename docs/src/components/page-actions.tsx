...
function LLMCopyButton({ content }: LLMCopyButtonProps) {
...
export function PageActions({ content, githubUrl }: PageActionsProps) {
  const pathname = usePathname();
  const markdownUrl = useMemo(() => {
    if (!pathname) {
      return "/llms.mdx/docs.md";
    }

    return `${pathname.replace("/docs", "/llms.mdx/docs")}.md`;
  }, [pathname]);

  return (
    <div className="flex flex-row flex-wrap items-center gap-2 border-b pb-6">
      <LLMCopyButton content={content} />
